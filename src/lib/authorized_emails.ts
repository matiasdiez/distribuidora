/**
 * authorized_emails.ts — Lista de emails autorizados para acceder a la app.
 *
 * Agregar aquí los emails de Google que deben tener acceso.
 * Solo modificar este archivo; no hace falta cambiar otra cosa.
 *
 * La comparación es case-insensitive (no importa mayúsculas/minúsculas).
 */
const AUTHORIZED_EMAILS: string[] = [
  // por ejemplo:
  // gerente@miempresa.com
  'matiasdiez@gmail.com',
  'matiasluisdiez@gmail.com',
];

export default AUTHORIZED_EMAILS;
