<script lang="ts">
  ...
  $: filteredBrands = allBrands.filter(b => {
    ...
    const matchDepot = activeDepotId === 'all' ||
      b.depotIds.has(activeDepotId as number);
  ...
  async function handleSaved()   { modalOpen = false; await refreshBrand(); }

  // ── Notas ─────────────────────────────────────────────────────
  function startNewNote() {
    editingNote = null; noteContent = ''; noteStatus = 'open';
    notePriority = 'normal'; noteDueDate = ''; noteIsTask = false; showNoteForm = true;
  }

  function startEditNote(n: BrandNote) {
    editingNote = n; noteContent = n.content; noteStatus = n.status;
    notePriority = n.priority; noteDueDate = n.due_date ?? '';
  ...
  async function saveNote() {
    ...
    try {
      if (editingNote?.id) {
        ...
        await saveBrandNote({
          brand: selectedBrand, content: noteContent.trim(),
          status:   noteIsTask ? noteStatus   : 'open',
          priority: noteIsTask ? notePriority : 'normal',
          due_date: noteIsTask && noteDueDate ? noteDueDate : null,
        });
      }
      showNoteForm = false; editingNote = null;
      await refreshBrand();
...
<style>
  ...
  .prio-low    { background: var(--green-dim, #1a2a1a); color: var(--green, #4ade80); border: 1px solid var(--green-dim, #1c3a28); }
  ...
  .prio-done   { background: var(--green-dim, #1a2a1a); color: #4ade80; }
  ...
  .prio-btn.active.prio-done   { border-color: #4ade80; color: #4ade80; background: var(--green-dim, #0d1f0d); }
  ...
  .prod-stock-btn { flex-shrink: 0; height: 32px; padding: 0 12px; border-radius: 5px; border: 1.5px solid var(--amber-dim, #b57a1a); background: var(--amber-bg, #1a1200); color: var(--amber); font-family: var(--font-ui); font-size: 13px; font-weight: 700; cursor: pointer; -webkit-tap-highlight-color: transparent; }
