<script lang="ts">
  ...
  type BrandRow = {
    name: string;
    productCount: number;
    depotIds: Set<number>; // depósitos en los que aparece esta marca
  ...
  // ── Estado global ─────────────────────────────────────────────
  let allBrands: BrandRow[] = []; // lista completa, cargada una vez
  let loading = true;
  let depots: Depot[] = [];

  // ── Filtros (puramente reactivos, sin async) ──────────────────
  let searchQuery: string = "";
  let activeDepotId: number | "all" = "all";

  // Filtrado 100% en memoria — instantáneo
  $: filteredBrands = allBrands.filter((b) => {
    const matchSearch =
      !searchQuery || b.name.toLowerCase().includes(searchQuery.toLowerCase());
    const matchDepot =
      activeDepotId === "all" || b.depotIds.has(activeDepotId as number);
    return matchSearch && matchDepot;
  });

  // ── Vista de marca seleccionada ───────────────────────────────
  ...
  // ── Carga inicial ─────────────────────────────────────────────
  // Una sola pasada sobre IndexedDB: trae todos los productos y
  // todos los lotes de una vez. Sin bucles de consultas individuales.
  onMount(async () => {
  ...
  async function loadAllBrands() {
    ...
    // Leer productos y lotes en paralelo — dos consultas totales
    const [products, stockEntries, notes] = await Promise.all([
      db.getAll("products"),
      db.getAll("stock_entries"),
      getBrandNotes(),
    ]);

    const now = Date.now();

    // Índice de lotes por product_id para lookup O(1)
    const stockByProduct = new Map<number, (typeof stockEntries)[0][]>();
    for (const e of stockEntries) {
      if (!stockByProduct.has(e.product_id))
        stockByProduct.set(e.product_id, []);
      stockByProduct.get(e.product_id)!.push(e);
    }

    // Índice de notas abiertas por marca
    const openNotesByBrand = new Map<string, number>();
    for (const n of notes) {
      if (n.status === "open") {
        openNotesByBrand.set(n.brand, (openNotesByBrand.get(n.brand) ?? 0) + 1);
      }
    }

    // Construir una fila por marca — todo en memoria, sin más consultas
    ...
    for (const p of products) {
      if (!brandMap.has(p.brand)) {
        brandMap.set(p.brand, {
          ...
          depotIds: new Set(),
          lastUpdated: null,
          freshnessClass: "f-none",
          freshnessLabel: "Sin datos",
          openNotes: openNotesByBrand.get(p.brand) ?? 0,
        });
      }
      const row = brandMap.get(p.brand)!;
      row.productCount++;
      if (p.depot_id != null) row.depotIds.add(p.depot_id);

      // Fecha más reciente de cualquier lote de este producto
      const lots = stockByProduct.get(p.id) ?? [];
      for (const l of lots) {
        const d = new Date(l.created_at);
        if (!row.lastUpdated || d > row.lastUpdated) row.lastUpdated = d;
      }
    }

    // Calcular frescura y ordenar
    const list = [...brandMap.values()];
    for (const row of list) {
      const { cls, label } = freshnessOf(row.lastUpdated, now);
      row.freshnessClass = cls;
      row.freshnessLabel = label;
    }
  ...
  async function refreshBrand() {
    ...
    brandProducts = all.filter((p) => p.brand === selectedBrand);
    brandNotes = await getBrandNotes(selectedBrand);
  }

  function closeBrand() {
    selectedBrand = null;
    // Refrescar frescura de la marca que acabamos de visitar
    loadAllBrands();
  }
  ...
  {#if !selectedBrand}
    <div class="header">
      <span class="header-title"
        ><Store size={16} strokeWidth={2} /> Marcas</span
      >
    </div>

    <!-- Búsqueda instantánea -->
    <div class="marcas-search-wrap">
      <span class="marcas-search-icon"
      ...
      <div class="depot-filter">
        ...
        {#each depots as d}
          <button
            ...
            on:click={() => (activeDepotId = d.id)}>{d.name}</button
          >
        {/each}
      </div>
    {/if}
    ...
    {:else}
      <div class="brand-list">
        {#each filteredBrands as b}
          <button class="brand-row" on:click={() => openBrand(b.name)}>
            <span
              class="freshness-dot {b.freshnessClass}"
              title={b.freshnessLabel}
            ></span>
            ...
            <span class="brand-meta">
              ...
              {#if b.openNotes > 0}
                <span class="note-badge"
                  ><StickyNote size={10} strokeWidth={2} /> {b.openNotes}</span
                >
              {/if}
            </span>
            <span class="freshness-label {b.freshnessClass}"
              >{b.freshnessLabel}</span
            >
      ...
      <div class="legend">
        <span class="legend-item"
          ><span class="freshness-dot f-today"></span> Hoy/Ayer</span
        >
        <span class="legend-item"
          ><span class="freshness-dot f-recent"></span> 2–3 d</span
        >
        <span class="legend-item"
          ><span class="freshness-dot f-old"></span> 4–7 d</span
        >
        <span class="legend-item"
          ><span class="freshness-dot f-stale"></span> +7 d</span
        >
        <span class="legend-item"
          ><span class="freshness-dot f-none"></span> Sin datos</span
        >
      </div>
    {/if}
    ...
    {#if brandNotes.length > 0 || showNoteForm}
      <div class="notes-section">
        <p class="notes-section-label">
          <StickyNote size={11} strokeWidth={2} /> Notas
        </p>
        ...
        {#each brandNotes as note}
          <div class="note-card" class:note-done={note.status === "done"}>
            <div class="note-top">
              <button
                class="note-check"
                on:click={() => toggleDone(note)}
                title={note.status === "done"
              ...
              <div class="note-actions">
                <button class="note-btn" on:click={() => startEditNote(note)}
            ...
            {#if note.due_date || note.priority !== "normal"}
              <div class="note-meta">
                ...
                {#if note.due_date}
                  <span
                    class="due-date"
                    class:overdue={isOverdue(note.due_date)}
                  >
                    <Calendar size={10} strokeWidth={2} />
                    {formatDate(note.due_date)}
                    {#if isOverdue(note.due_date)}<span class="overdue-label">
                        vencida</span
                      >{/if}
                  </span>
                {/if}
        ...
        {#if showNoteForm}
          <div class="note-form">
            ...
            {#if noteIsTask}
              <div class="task-fields">
                ...
                <div class="task-field">
                  ...
                  <input
                    class="input-field task-date"
                    type="date"
                    bind:value={noteDueDate}
                  />
                </div>
                {#if editingNote}
                  <div class="task-field">
                    ...
                    <div class="prio-selector">
                      <button
                        class="prio-btn"
                        class:active={noteStatus === "open"}
                        on:click={() => (noteStatus = "open")}>Pendiente</button
                      >
                      <button
                        class="prio-btn prio-done"
                        class:active={noteStatus === "done"}
                        on:click={() => (noteStatus = "done")}>Hecha</button
                      >
                    </div>
                  </div>
                {/if}
            ...
            <div class="note-form-actions">
              <button
                class="btn btn-ghost"
                on:click={() => (showNoteForm = false)}>Cancelar</button
              >
              <button
              ...
              >
                {#if noteSaving}<span class="spin-sm">⟳</span>{:else}<Check
                    size={14}
                    strokeWidth={3}
                  />{/if}
      ...
      {#each brandProducts as p (p.id)}
        <div class="prod-row">
          <div class="prod-info">
            ...
            {#if p.weight_qty}<span class="prod-weight">{p.weight_qty}</span
              >{/if}
          </div>
          {#if depotId}
            <button class="prod-stock-btn" on:click={() => openModal(p)}
              >+ Stock</button
            >
          {/if}
        </div>
...
<style>
  ...
  .prio-btn.active.prio-low {
    border-color: #4ade80;
    color: #4ade80;
    background: #0d1f0d;
  }
  .prio-btn.active.prio-normal {
    border-color: #818cf8;
    color: #818cf8;
    background: #0d0d1f;
  }
  .prio-btn.active.prio-high {
    border-color: var(--red);
    color: var(--red);
    background: #1f0d0d;
  }
  .prio-btn.active.prio-done {
    border-color: #4ade80;
    color: #4ade80;
    background: #0d1f0d;
  }
  ...
  .note-form-actions .btn-ghost {
    flex-shrink: 0;
  }
  .note-form-actions .btn-primary {
    flex: 1;
  }
  .spin-sm {
  ...
  .marcas-search-wrap:focus-within {
    border-color: var(--amber);
  }
  .marcas-search-icon {
    position: absolute;
  ...
  .marcas-search-input::placeholder {
    color: var(--text-lo);
  }
  .marcas-search-input::-webkit-search-cancel-button {
    display: none;
  }
  .marcas-search-clear {
    position: absolute;
